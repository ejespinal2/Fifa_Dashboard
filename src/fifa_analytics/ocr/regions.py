...
PLAYER_SUMMARY_REGIONS = {
    ...
    # empty gap between them. The player values are read as one vertical
    # strip in a single OCR pass (see extract.read_number_column), which is
    # far more reliable than cropping and OCR'ing 17 one-character images
    # separately -- so these y-bounds just have to span the list; a small
    # offset no longer drops or clips individual rows.
